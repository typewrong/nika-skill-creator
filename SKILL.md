...
本技能是 Codex 标准技能，用于在本仓库内创建/更新符合 Nika 平台约束的技能文档，并提供可复制到 Nika 的主文档与子文档内容。

重要区分：
- 本技能本体不需要遵循 Nika 约束（因为它运行在 Codex/本地仓库环境）。
...
```bash
python3 codex-skills/nika-skill-creator/scripts/init_nika_skill.py "你的目标技能中文名"
```

2. 校验目标技能是否满足 Nika 约束与本仓库约定：

```bash
python3 codex-skills/nika-skill-creator/scripts/validate_nika_skill.py "skills/你的目标技能中文名"
```
...
默认交付行为：
- 在仓库中生成/更新目标技能目录与文档
- 在对话中输出：目标技能概要、子文档清单、校验结果摘要（引用 @validation-checklist 的检查维度）
...
```bash
python3 codex-skills/nika-skill-creator/scripts/validate_nika_skill.py "skills/目标技能中文名"
```
...
- @nika-spec：Nika 平台约束（目标技能必须遵循）
- @main-doc-template：目标技能主文档骨架模板
- @sub-doc-template：目标技能子文档模板
- @validation-checklist：人工校验清单（与脚本校验对齐）
